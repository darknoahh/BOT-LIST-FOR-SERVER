...
let westralogkanal = await db.fetch(`westralogkanal_${message.guild.id}`, kanal3.id);
  ...
  let sebep9 = args[11]
	...
	if (!botisim) return message.channel.send(`❌ You must write the ID of the bot.`).then(x => x.delete({timeout: 3000}))
  ...
  message.delete()
   const westrabumbeyyyy = new Discord.MessageEmbed()
    ...
    .setTimestamp()
    .setDescription(`❌ <@${botisim}> The bot named was rejected.`)
		dark.channels.cache.get(log).send(westrabumbeyyyy);
		message.channel.send(`✅ You refused the boat.`).then(x => x.delete({timeout: 3000}))
};
...
exports.help = {
  ...
  usage: 'bot-reject <bot name> - <reason>'
};